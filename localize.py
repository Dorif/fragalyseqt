...
# Or no locale set, like OpenBSD.
    if lang is None or ("en" or "C.UTF-8" or "POSIX") in lang:
        ...
        iface['infoboxtxt'] = "FragalyseQt version 0.4.1, codename\
        \"Jeffreys\".\n\nThis program version supports analysis of up to 8 \
        different fluorescense channels simultaneously, selective channel \
        hiding, non-Latin run names and can correctly handle damaged files, \
        exporting peaks locations, areas, sizes, FWHM's and channel names in \
        CSV for any *.FSA/*.HID files and export in CSV internal analysis data\
        for *.FSA/*.HID files generated by ABI 3500 and SeqStudio series \
        equipment.\n\nPeaks areas are calculated assuming they are Gaussian \
        peaks.\n\nLicensed under GNU AGPL version 3.\n\nIf you wish to contact\
        author for any reason - please write at dorif11@gmail.com"
        ...
        from FSA files generated by ABI 3500 and SeqStudio family sequencers!"
        ...
        \"Джеффрис\".\n\nЭта версия программы поддерживает одновременный \
        анализ до 8 каналов флуоресценции, избирательное сокрытие каналов, \
        не-латинские имена запусков, может правильно работать с повреждёнными \
        файлами, поддерживает экспорт имён каналов, положений, площадей, \
        размеров и FWHM пиков в формат CSV для любого файла *.FSA/*.HID и \
        экспорт данных внутреннего анализа данных для файлов *.FSA/*.HID, \
        полученных на инструментах ABI 3500 и SeqStudio.\n\nПлощади пиков \
        рассчитываются исходя из того, что пики являются гауссовыми\
        .\n\nЛицензировано под GNU AGPL версии 3.\n\nЕсли по какой-либо \
        причине вы хотите проконтактировать с автором - пишите, пожалуйста, на\
        dorif11@gmail.com"
        ...
        экспортированы только из файлов FSA, полученных на ABI 3500 \
        или семействе SeqStudio!"
        ...
        определения размера! Пожалуйста, попробуйте другие!"
    elif "ua" in lang:
        ...
        .\n\n Версія програми підтримує одночасний аналіз до 8 каналів \
        флуоресценции, вибіркове приховування каналів, не-латинські імена \
        запусків, може правильно працювати з пошкодженими файлами, підтримує \
        експорт імен каналів, положень, площадей, розміру і FWHM піков у \
        формат CSV для будь-якого файла *.FSA/*.HID та експорт даних \
        внутрішнього аналізу даних для файлів *.FSA/*.HID, отриманих на \
        інструментах ABI 3500 и SeqStudio.\n\nПлощаді піков розраховуються \
        виходячи з того, що піки є гауссовими.\n\nЛіцензовано під GNU AGPL \
        версії 3.\n\nЯкщо з будь-якої причини ви хочете проконтактувати з \
        автором - пишіть, будь ласка, на dorif11@gmail.com"
        ...
        експортовані тільки із файлів FSA, отриманих на ABI 3500 або \
        сімействі SeqStudio!"
        ...
        визначення розміру! Будь ласка, спробуйте інші!"
    elif "ro" in lang:
        ...
        .\n\nAceastă versiune permite analiza simultană a pînă la 8 canale de \
        fluorescență, ascunderea selectivă a canalelor, nume non-latine a \
        runurilor, poate funcționa corect cu fișiere deteriorate, permite \
        exportarea numelor de canale, locațiile, suprafețele, dimensiunile și \
        FWHMurile a vîrfurilor în formatul CSV, pentru orice fișierul de tip \
        *.FSA/*.HID și exportarea în formatul CSV a datelor analizei interne \
        pentru fișiere de tip *.FSA/*.HID generate de echipamentele de model \
        ABI 3500 și SeqStudio.\n\nSuprafețele vîrfurilor, sunt calculate \
        presupunînd că vîrfurile sunt gausiene.\n\nLicențiat sub GNU AGPL \
        versiunea 3.\n\nDacă doriți să contactați autorul, pentru orice \
        întrebare - scrieți, vă rog, la adresa dorif11@gmail.com"
        ...
        numai pentru fișierile FSA generate de echipamentele de model \
        ABI 3500 și SeqStudio!"
        ...
        dimensionare incorectă! Vă rugăm să încercați altele!"
    elif "fr" in lang:
        iface['ch_inact_msg'] = "Canal inactif"
        iface['aboutbtn'] = "Au sujet de"
        iface['infoboxtxt'] = "FragalyseQt version 0.4.1, alias \"Jeffreys\".\n\
        \nCette version du programme prend en charge l'analyse simultanée de \
        jusqu'à 8 canaux de fluorescence différents, le masquage sélectif des \
        canaux, les noms d'exécution non latins et peut gérer correctement les\
        Fichiers endommagés, exporter les emplacements des pics, zones, \
        tailles, FWHM et noms de canal au format CSV pour tous les Fichiers \
        *.FSA/*.HID et exportation au format CSV des données d'analyse interne\
        pour les Fichiers *.FSA/*.HID générés par les équipements des séries \
        ABI 3500 et SeqStudio.\n\nLes zones de pics sont calculées en \
        supposant qu'il s'agit de pics gaussiens.\n \nLicence sous GNU AGPL \
        version 3.\n\nSi vous souhaitez contacter l'auteur pour une raison \
        ...
        être exportées qu'à partir de Fichiers FSA générés par les \
        séquenceurs de la famille ABI 3500 et SeqStudio!"
        ...
        iface['bcd'] = "Correction de la ligne de base et \
        suppression du bruit."
        iface['wrongsizing'] = "Marqueur de dimensionnement ou méthode de \
        dimensionnement incorrect ! S'il vous plaît, essayez-en d'autres !"
    elif "bg" in lang:
        ...
        .\n\nТази версия на програмата поддържа едновременен анализ до 8 \
        флуоресцентни канала, селективно скриване на канали, не-латински имена\
        на изпълнения, може да работи правилно с повредени файлове, поддържа \
        експорт на имена на канали, позиции, площите, размери и FWHM пикове в \
        CSV формат за всеки *.FSA/*.HID файл и експортиране на вътрешни данни \
        за анализ на данни за *.FSA/*.HID файлове, получени с ABI 3500 и \
        инструменти SeqStudio.\n\nПлощите на пиковете се изчисляват, като се \
        приема, че пиковете са гаусови.\n\nЛицензирано под GNU AGPL версия 3.\
        \n\nАко по някаква причина искате да се свържете с автора, моля, \
        ...
        експортирани само от файлове FSA, придобити на ABI 3500 или \
        фамилията SeqStudio!"
        ...
        iface['wrongsizing'] = "Неправилен маркер за оразмеряване или метод за\
        оразмеряване! Моля, опитайте други!"
