...
Since version 0.5, allele binning is supported using panel files from GeneMapper (tab-delimited`*_Panel*.txt`
with optional companion `*_Bins*.txt`) and GeneMarker (`.xml`). When a panel is loaded,sized peaks are
automatically assigned allele designations; peaks outside all known bins are labelled OL (off ladder). Also
since version 0.5, export to **CODIS 3.2 CMF XML** format is supported: open the export dialog with
`Ctrl+Shift+C`, select which samples to include, assign each a specimen category and ID, fill in lab metadata,
and FragalyseQt will generate a schema-valid `CODISImportFile` ready for database submission. All 29 CODIS
specimen categories are supported; locus data is extracted automatically from loaded panel assignments.
