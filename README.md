...
`*_Panels_*.txt` with optional companion `*_Bins_*.txt`) and GeneMarker (`.xml`). When a panel is loaded,
sized peaks are automatically assigned allele designations; peaks outside all known bins are labelled OL
(out of ladder).
...
Currently, I plan to further improve binning and genotyping support.
