...
# Closing file to save memory and avoid unexpected things.
            if tmprecord.annotations["abif_raw"]["DATA1"] is None:
                # Assuming what it may be HID file.
                try:
                    HIDfile = open(fname, "rb")
                    s = HIDfile.read()
                    tmpkeys = tmprecord.annotations["abif_raw"].keys()
                    if "MODL1" in tmpkeys and tmprecord.annotations["abif_raw"]["MODL1"] is None:
...
# Assuming no more than 8 dyes are met at once.
            ...
            elif ("RunN1" and "DySN1" and "HCFG3") in keysarray and abif_raw["DySN1"] is not None and (b'\xd1\xca' in abif_raw["DySN1"] or b'.avt' in abif_raw["RunN1"]) and abif_raw["HCFG3"] == b'3130xl':
    ...
    def findpeaks(self):
        ...
        while channumber < DN:
            ...
            chN[channumber] = [Dye[channumber]]*len(chP[channumber][0])
            ...
            if should_sizecall and len(chP[channumber][0]) != 0:
                if dgr == 0:
    ...
    def export_csv(self):
        ...
        if expbox.focusWidget().objectName() == "CSV":
            ...
            if "Peak1" in keysarray and abif_raw["Peak1"] is not None:
                ...
                msgbox(ifacemsg['unsupportedeq'], ifacemsg['unsupportedeqmsg'], 1)
        if do_export:
